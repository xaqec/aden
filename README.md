...
2. `aden-portfolio` klasörünün **içindeki dosyaları** GitHub deponuzun köküne kopyalayın. `index.html` depo kökünde olmalı.
...
- `css/style.css`: Renkler, yerel yazı tipleri, duyarlı düzen, etkileşimler ve azaltılmış hareket desteği.
- `js/main.js`: Mobil menü, Escape ile kapatma ve geçerli bölüm göstergesi.
...
Renkler `style.css` başındaki `:root` değişkenlerinden yönetilir. Yerel kaynak yolları görecelidir: `./css/`, `./js/`, `./assets/`. Başlarına `/` eklemeyin; bu değişiklik GitHub Pages depo alt dizinini bozabilir. Telif yılı bilerek 2026 olarak sabittir.
